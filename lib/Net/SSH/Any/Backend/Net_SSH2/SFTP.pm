...
sub _pipe_error {
    my $self = shift;
    my $error =  $self->{pipe}->error;
    join (": ", @_, sprintf("%s (%d)", $error, $error));
}

sub _conn_failed {
    my ($self, $sftp, @msg) = @_;
    $sftp->_conn_failed($self->_pipe_error(@msg))
}


sub _conn_lost {
    my ($self, $sftp, @msg) = @_;
    $sftp->_conn_lost(undef, undef, $self->_pipe_error(@msg));
}
